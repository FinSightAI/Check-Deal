    ...
    <div className="space-y-6">
        ...
        <div className="space-y-2 text-sm">
          {[
            ...
            <div key={r.label} className="flex justify-between">
              ...
              <span className="font-medium text-slate-700">{formatCurrency(r.val, 'BRL')}</span>
            </div>
          ))}
          <div>
          ...
          <div className="flex justify-between font-semibold pt-2 border-t border-slate-100">
            ...
            <span className="text-blue-700">{formatCurrency(totalBasis, 'BRL', true)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        ...
        <div>
          <div className="flex justify-between mb-1 text-sm">
            ...
            <span className="font-bold text-slate-800">{formatCurrency(exitPrice, 'BRL', true)}</span>
          </div>
      ...
      <div className={`rounded-xl p-5 border-2 ${gcapTax === 0 ? 'bg-emerald-50 border-emerald-300' : 'bg-white border-slate-200'}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          {[
            { label: 'Gross gain', val: formatCurrency(netGain, 'BRL', true), color: 'text-slate-800' },
            { label: 'GCAP tax', val: formatCurrency(gcapTax, 'BRL', true), color: gcapTax > 0 ? 'text-red-600' : 'text-emerald-600' },
            { label: 'Effective rate', val: `${effectiveRate.toFixed(1)}%`, color: 'text-slate-700' },
            { label: 'Net proceeds', val: formatCurrency(netProceeds, 'BRL', true), color: 'text-blue-700' },
          ].map(m => (
            ...
            {sensitivityPrices.map(r => (
              <tr key={r.multiple} className={r.price === exitPrice ? 'bg-amber-50' : ''}>
                <td className="px-4 py-2.5 font-medium text-slate-700">
                  {formatCurrency(r.price, 'BRL', true)}
                  <span className="text-xs text-slate-400 ml-1">×{r.multiple}</span>
                </td>
                <td className="px-4 py-2.5 text-right text-slate-600">{formatCurrency(r.gain, 'BRL', true)}</td>
                <td className="px-4 py-2.5 text-right text-red-600">{r.tax > 0 ? formatCurrency(r.tax, 'BRL', true) : '—'}</td>
                <td className="px-4 py-2.5 text-right font-semibold text-blue-700">{formatCurrency(r.net, 'BRL', true)}</td>
              </tr>
            ))}
          </tbody>
        </table>
